    ...
    <script>
        window.addEventListener('keydown', function(e) {
            if (e.key == 's' && e.ctrlKey) {
                e.preventDefault();
                Livewire.emit('toggleModal');

            }
        })

        window.addEventListener('focusOnSearchInput', () => {
...
<div wire:keydown.escape="$set('modal',false)" wire:keydown.window="toggleModal">
    ...
    @if ($modal)
        <div id="nav-search" wire:click.self="$set('modal',false) "
            class="fixed top-0 bottom-0 left-0 right-0 z-40 bg-black/50">

            {{-- close button  --}}
            <div id="nav-search-close" wire:click="$set('modal',false)"
                class="absolute top-11 right-8 p-3 rounded-full text-white bg-gray-700/30 hover:cursor-pointer hover:bg-gray-700 transition duration-300 z-50">
                ...
                <div class="w-1/2 mx-auto flex flex-col justify-center items-center gap-4">
                    <form class="flex items-center w-full">
                        ...
                        <div class="relative w-full">
                            <div
                                class="absolute text-gray-500 inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                            ...
                            <input type="text" wire:model="search" id="simple-search" autocomplete="off"
                        ...
                        <div class="p-8 bg-white w-full rounded-md">
                            <div class="grid grid-cols-5">
                                @forelse ($products as  $product)
                                    <div
                                        class="flex flex-col gap-3 p-3 max-h-52 rounded-md hover:scale-110 hover:shadow-md transition duration-500">
                                        ...
                                        <div class="space-y-1">
                                            <p
                                                class="font-roboto uppercase text-xs tracking-wide font-light text-gray-700 truncate">
                                                {{ $product->name }}</p>
                                            <p
                                                class="font-roboto uppercase text-xs tracking-wide font-light text-gray-700">
                                                LE
                                ...
                                @for ($i = 0; $i < 5; $i++)
                                    <div role="status" class="animate-pulse mt-10 pl-4" wire:loading>
                                        <div
                                            class="h-2.5 bg-gray-400 rounded-full dark:bg-gray-700 max-w-[640px] mb-2.5 mx-auto">
                                        </div>
                                        <div
                                            class="h-2.5 mx-auto bg-gray-400 rounded-full dark:bg-gray-700 max-w-[540px]">
                                        </div>
                                        <div class="flex items-center justify-center mt-4">
                                            <svg class="w-10 h-10 mr-2 text-gray-400 dark:text-gray-700"
                                            ...
                                            <div class="w-20 h-2.5 bg-gray-400 rounded-full dark:bg-gray-700 mr-3">
                                            </div>
                            ...
                            @if ($products->count())
                                <div class="text-center pt-8">
                                    <a href="{{ route('products.index') }}">
                                        <span class="text-gray-500 text-sm hover:text-gray-600 underline ">See more
                                            products</span>
                                    </a>
                                </div>
                            @endif
