                    ...
                    @foreach ($options as $name => $optionValues)
                        ...
                        <div class="space-y-2 ">
                            <button id="{{ $name }}" class="filter flex items-center justify-between w-full">
                                <span
                                    class="font-extralight text-sm text-gray-500 capitalize">{{ $name }}</span>
                                <span id="sign" class="font-normal text-2xl text-gray-500">+</span>
                            </button>
                                ...
                                <div class="flex flex-col pb-4 ">
                                    ...
                                    @foreach ($optionValues as $key => $optionValue)
                                        <div class="flex items-center  rounded">
                                            <input wire:model='filterValues. {{ $name . $key }}'
                    ...
                    <div class="space-y-2 ">
                        ...
                        <div data-filter-name="sort" class="max-h-0 overflow-hidden transition-all duration-700">
                            <div class="flex flex-col pb-4">
                                ...
                                <div class="flex items-center mb-2">
                                    <input id="default-radio-2" type="radio" wire:model="sortByValue"
                                        value="priceLowToHigh" wire:click="sortBy"
                                        class="w-5 h-5 text-gray-400 bg-gray-100 rounded-full border-0 focus:ring-0">
                                    <label for="default-radio-2"
                                        class="py-1 ml-2 w-full text-sm capitalize tracking-wide text-gray-500">Price,
                                        Low
                                        to High</label>
                                </div>

                                <div class="flex items-center mb-2">
                                    <input id="default-radio-3" type="radio" wire:model="sortByValue"
                                ...
                                <div class="flex items-center mb-2">
                                    <input id="default-radio-4" type="radio" wire:model="sortByValue" value="onSale"
                                        wire:click="sortBy"
                                        class="w-5 h-5 text-gray-400 bg-gray-100 rounded-full border-0 focus:ring-0">
                            ...
                            <div class="inner-image-wrapper relative">
                                ...
                                @if ($product->productSkus->sum('quantity') <= 0)
                                    <span
                                        ...
                                        Out</span>
                                @else
                                    @if ($product->is_hot == 'Hot')
                                        <span
                                            class="absolute top-4 left-4 uppercase py-1 px-3.5 font-roboto bg-red-400 text-white text-sm">HOT</span>
                                    @endif
                                    @if ($product->sale != null)
                                        <span
                                            class="absolute top-4 {{ $product->is_hot == 'Hot' ? 'left-[85px]' : 'left-4' }} uppercase py-1 px-3.5 font-roboto bg-gray-700 text-white text-sm">Save
